...
import { multipleTabsKey, removeToken, type DataInfo } from "@/utils/auth";
...
const routes = [
  ...
  },
  {
    path: "/newScreen",
    name: "NewScreen",
    component: () => import("@/views/newScreen/screen.vue"),
    hidden: true,
    meta: {
      title: "新港口大屏",
      rank: 9,
...
router.beforeEach((to: ToRouteType, _from, next) => {
  ...
  if (!externalLink) {
    to.matched.some(item => {
      ...
      if (Title)
        document.title = `${transformI18n(item.meta.title)} | ${transformI18n(Title)}`;
      else document.title = transformI18n(item.meta.title);
    });
  }
  /** 如果已经登录并存在登录信息后不能跳转到路由白名单，而是继续保持在当前页面 */
  function toCorrectRoute() {
    const currentRoute = useMultiTagsStoreHook().multiTags.find(
      item => item.path === to.fullPath
    );
    if (currentRoute && Object.keys(to.query).length === 0) {
      if (whiteList.includes(to.fullPath)) {
        ...
        next({ path: currentRoute.path, query: currentRoute.query });
      } else {
        next();
      }
    } else {
...
router.afterEach(() => {
  /**
   *  判断是否是离开/workflow/writeWorkForm页面
   *  是的话：就保存当前页面的url参数，以便下次进入/workflow/writeWorkForm页面时使用;
   * */
  // if (router.currentRoute.value.path === "/workflow/writeWorkForm") {
  //   storageLocal().setItem("historyQuery", {
  //     [router.currentRoute.value.path]: router.currentRoute.value.query
  //   });
  // }
  NProgress.done();
});
