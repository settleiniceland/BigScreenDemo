  ...
  <el-form size="small">
    ...
    <el-form-item>
      <el-radio v-model="radioValue" :label="3">
        周期从
        <el-input-number v-model="cycle01" :min="fullYear" :max="(maxFullYear - 1) >= 0 ? maxFullYear - 1 : 0" /> -
    ...
    <el-form-item>
      <el-radio v-model="radioValue" :label="4">
        从
        <el-input-number v-model="average01" :min="fullYear" :max="(maxFullYear - 1) >= 0 ? maxFullYear - 1 : 0" /> 年开始，每
...
// 定义 watch 观察器
const stopWatchCronYear = watch(
  () => props.cron.year,
  (value) => changeRadioValue(value)
);

const stopWatchRadioChange = watch([radioValue, cycleTotal, averageTotal, checkboxString], () => onRadioChange());
...
// 使用 onUnmounted 钩子清理资源，防止内存泄漏
onUnmounted(() => {
  stopWatchCronYear(); // 停止 watch
  stopWatchRadioChange(); // 停止 watch
});
</script>
