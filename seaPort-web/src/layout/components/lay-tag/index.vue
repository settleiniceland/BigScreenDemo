...
function dynamicRouteTag(value: string, route: RouteLocationNormalizedLoadedGeneric): void {
  ...
  function concatPath(arr: object[], value: string) {
    if (!hasValue) {
      ...
      // 判断query是否相等,不相等则要跟新query
      const index = multiTags.value.findIndex(item => item.path === value);
      if (!isEqual(multiTags.value[index].query, route.query) && !isAllEmpty(route.query)) {
        multiTags.value[index].query = route.query;
      }
    }
  }
...
onMounted(() => {
  ...
  //  接收侧边栏切换传递过来的参数
  emitter.on("changLayoutRoute", ({ indexPath, route }) => {
    dynamicRouteTag(indexPath, route);
    setTimeout(() => {
...
onBeforeUnmount(() => {
  // 解绑`tagViewsChange`、`tagViewsShowModel`、`changLayoutRoute`公共事件，防止多次触发x
